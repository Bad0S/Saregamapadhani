...
public class EnemyBehaviourTuto : MonoBehaviour {
	...
	private void OnCollisionEnter2D(Collision2D other){
		if (other.gameObject.tag == "Player") 
		{
			if(isJumping == true){
				...
				GetComponent<BoxCollider2D> ().isTrigger = true;
				grabbed = true;
				target.GetComponent <PlayerTuto> ().grabbed=true;
				target.GetComponent<PlayerTuto> ().GrabUngrab ();
			}
		}
	}
	...
	private void OnTriggerEnter2D(Collider2D other)
	{
		if (other.tag == "PlayerAttack") 
		{
			...
			if(grabbed == true)
			{
				GameObject.FindGameObjectWithTag ("Player").GetComponent <PlayerTuto>().grabbed = false ;
				GameObject.FindGameObjectWithTag ("Player").GetComponent <PlayerTuto>().GrabUngrab () ;
