...
public class CameraBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (target.position.x <= cam1.position.x - 0.8f || target.position.x >= cam1.position.x + 0.8f)
        {
            transform.position = Vector3.Lerp(transform.position, targetPositionX, Time.deltaTime * Camspeed);
        }
        if (target.position.y <= cam1.position.y - 0.45f || target.position.y >= cam1.position.y + 0.45f)
        {
